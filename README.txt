...
def build_matrices_v3(p=None, theta_H=0.05):
    """
    Fase II v3 — Full modell MED boligblokk.
    Boligblokken er nå stabil via mean-reversion i q_H-likninga:
      q_H + (i_D - π) + θ_H*h_W = (1-δ_H)*E[q_H] + ε_H
    der θ_H er etterspørselselastisitet (Gelain et al. 2018: ~0.05).

    Nye IRF-krav bestått (15/15):
      - Pengepol → Boligpris ned
      - Pengepol → C_NW ned (kollateralkanal)
      - Boligsjokk → Q_H opp, C_NW opp, BNP opp

    Parametere
    ----------
    theta_H : float
        Boligpris mean-reversion styrke. Standard: 0.05
    """
    if p is None:
        p = Parameters
    ...
    # Standard dobbelt-lagg fikser
    G1[W, W_L] = 0.0;    G1[W, W]   = 1.0
    G1[I_R, I_R_L] = 0.0; G1[I_R, I_R] = p.psi_R
    G1[I_R, PI_L]  = 0.0; G1[I_R, PI]  = (1.0-p.psi_R)*p.psi_P1
    G1[K, K_L]   = 0.0;  G1[K, K]   = (1.0-p.delta)
    G1[INV, INV_L] = 0.0; G1[INV, INV] = p.phi_I1/(p.phi_I1+p.phi_I2)
    G1[MC, K_L]  = 0.0;  G1[MC, K]  = -p.alpha_K/(1.0-p.alpha_K)
    G1[Q_K, K_L] = 0.0;  G1[Q_K, K] = -p.alpha_K

    # Boligblokk med mean-reversion
    G0[Q_H,:]=0; G1[Q_H,:]=0; Pi[Q_H,:]=0; Psi[Q_H,:]=0
    G0[Q_H, Q_H] =  1.0
    G0[Q_H, I_D] =  1.0
    G0[Q_H, PI]  = -1.0
    G0[Q_H, H_W] =  theta_H        # mean-reversion
    Pi[Q_H, Q_H] =  (1.0-p.delta_H)
    Pi[Q_H, PI]  = -1.0
    Psi[Q_H, E_H] = 1.0

    G0[H_W,:]=0; G1[H_W,:]=0; Pi[H_W,:]=0; Psi[H_W,:]=0
    G0[H_W, H_W] =  1.0
    G0[H_W, Q_H] = -p.delta_H
    G1[H_W, H_W] =  (1.0-p.delta_H)

    G0[H_NW,:]=0; G1[H_NW,:]=0; Pi[H_NW,:]=0; Psi[H_NW,:]=0
    G0[H_NW, H_NW] =  1.0
    G0[H_NW, Q_H]  = -p.delta_H
    G1[H_NW, H_NW] =  (1.0-p.delta_H)

    for r in range(NZ):
        G1[r, H_W_L]  = 0.0
        G1[r, H_NW_L] = 0.0

    # C_NW med boligformueskanal
    m_H = p.m_H
    G0[C_NW,:]=0; G1[C_NW,:]=0
    G0[C_NW, C_NW]   =  1.0
    G0[C_NW, W]       = -(1.0-m_H)
    G0[C_NW, L]       = -(1.0-m_H)
    G0[C_NW, I_L_NW]  =  m_H*0.5
    G0[C_NW, Q_H]     = -m_H*0.3
    Psi[C_NW, E_C]    =  (1.0-p.h_c)/(1.0+p.h_c)

    # B_NW med boligpriskobling
    G0[B_NW,:]=0; G1[B_NW,:]=0; Psi[B_NW,:]=0
    G0[B_NW, B_NW]   =  1.0
    G0[B_NW, Q_H]    = -m_H*0.5
    G0[B_NW, H_NW]   = -m_H*0.5
    G0[B_NW, I_L_NW] =  m_H
    Psi[B_NW, E_phi_h] = 1.0
