...
| 2026-06-04 | **kj50: endogen risikopremie i UIP estimert (Alt A, 192k trekk)** | `build_matrices_rpendo` (NZ=51). Data støtter premien: κ_pe=0.043±0.016, ρ_pe=0.919±0.018 (godt identifisert). **Løser RER-q12-fortegnsskiftet** (kj41 +0.12 → kj50 −0.25 ≈ NB −0.20) og inflasjonspersistens. MEN psi_R presses til tak 0.99 (begrensning 7 reaktivert) → I_R overpersisterer → 16pt NB-RMSE 0.374 (kj41 0.295). PSRF=1.002, ESS/n=0.0046. Oppfølging: bryte psi_R↔premie-korrelasjon (PE). Se `docs/oppgaver/transmisjon_rer_diagnose.md` §6. |
| 2026-06-04 | **kj51: psi_R pinnet=0.949 → observasjonsekvivalens avdekket** | Med psi_R fast blir endogen premie svakt identifisert (κ=0.094±0.094, ρ_pe=0.39↓) og persistensen migrerer til EKSOGEN risikopremie (rho_rp=0.91↑). Aggregat-RMSE best (0.281) men RER-q12-fikset tapt (+0.14). **Konklusjon: endogen premie, psi_R og rho_rp er ikke separat identifiserbare med 14 observabler (Spor C6/begrensning 7).** Løsning krever flere observasjonsserier (NIBOR-OIS/terminpremie/kredittspread) eller aksept som begrensning. PSRF=1.12 (borderline). Se `transmisjon_rer_diagnose.md` §7, `kj51_vs_nb.png`. |
| 2026-06-04 | **kj52: i_3m anker pengemarkedspremie + sigma_prem aktivert** | Funn: premie-sjokket E_prem var inaktivt (Q=0) i alle tidligere kjøringer → EPS_PREM død tilstand; nå aktivert (sigma_prem estimert, N=22). i_3m løste premie-identifikasjonen (lp −3114 vs kj50 −3306) og κ_rp_endo robust (0.042±0.015). MEN psi_R driver fortsatt til 0.99 — pengemarkedspremien er et annet objekt enn FX-premie↔psi_R-floken. RER best av alle (q4≈−1.01, q12 −0.29), men I_R overpersisterer → RMSE 0.376. PSRF=1.004, ESS_min=750. **Å bryte floken krever FX-spesifikk observabel (terminpremie/cross-currency basis).** Se `transmisjon_rer_diagnose.md` §8. |
| 2026-06-04 | **FX-sporet AVSLUTTET — kj41 forblir referanse** | PE-beslutning: aksepter RER/FX-premie-identifikasjonsgrensen som dokumentert begrensning 8 (kj50–52: endogen FX-premie er datastøttet og fikser RER, men observasjonsekvivalent med renteglatting gitt 14 observabler; konstruert UIP-proxy for svak; ren FX-serie ikke i API-ene). PARAM_PRIORS tilbakestilt til kj41-default (N=19), premie-sjokket deaktivert i build_Q. **Fase 3 (analyseverktøy) kan starte med kj41 som referanseestimat.** Se `transmisjon_rer_diagnose.md` §9 + `fase05_begrensningsdokument.md` §8. |
| 2026-06-04 | **GEORG-læringssteg godkjent og implementert (NZ_GEORG=64)** | `build_matrices_georg()` lagt til ved siden av v3 (urørt) for å isolere om NB-avviket skyldes politikkregel eller transmisjon. Funn (mot kj41-baseline + NB Figur 1): **TRANSMISJON dominerer.** Identiske NB-GEORG-koeffisienter gir RMSE(16pt)=0.348 mot NB (mimicking 0.295), avvik konsentrert i RER og lang-horisont-persistens → siden regelen er lik NBs isolerer dette transmisjonsforskjellen. GEORG halverer I_R.q12-feilen (begrensning 6) men når ikke NBs fortegnsskifte. Se `docs/oppgaver/GEORG_laeringssteg_plan.md` §11 + `data/results/georg_vs_nb.png`. |
