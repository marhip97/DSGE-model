"""
[STAT] Kjøring 12 — akkumulerende MCMC med A6 (sigma_A fri) + A7 (psi_W mimicking rule).

Modellrettelser fra Spor A (PE-godkjent 2026-05-21):
  A6: sigma_A fristilt — K&M Tabell 9 estimerer sigma_A
      Prior: Normal(0.010, 0.004) på (0.002, 0.050)
      Forventet: rho_A → ~0.80 (K&M) når TFP-kanalen er riktig identifisert
  A7: psi_W-ledd i mimicking rule — K&M §2.13, psi_W=0.871
      Forventet: sterkere inflasjonsdynamikk → bedre KPI-respons

20 frie parametre (kj10: 19):
  Ny: sigma_A
  Fortsatt fast: h_c=0.938, phi_B=0.0016, phi_O=0.15
  Fri: phi_I1

Akkumuleringsstrategi (identisk med kjøring 10/11):
  Kald start fra kj10-posterior + sigma_A=0.010 startpunkt.
  Partial lagres hvert 1000 trekk.
"""
...
PARTIAL_PATH = ROT / "data" / "results" / "chain_kj12_prod_partial.npy"
TEMP_PREFIX  = str(ROT / "data" / "results" / "chain_kj12_temp")
OUT_PATH     = ROT / "data" / "results" / "chain_kj12_prod_posterior.json"

print(f"Kjøring 12 akkumulerende — {N_PARAMS} frie parametre")
print(f"Modellrettelser: A6 (sigma_A fri), A7 (psi_W={psi_W:.3f} i mimicking rule)")
...
# Beholder kj11s dedikerte rho_rp-blokk + sigma_A i AR-blokken
...
if PARTIAL_PATH.exists():
    ...
    for i, n in enumerate(PARAM_NAMES):
        if n in prev_summ:
            ...
            # A6: nytt parameter, start fra K&M-tilnærming (0.010)
            theta_start[i] = 0.010
            post_std[i]    = 0.004
        else:
    ...
    # rho_A: med sigma_A fri forventer vi at rho_A løfter seg mot K&M ~0.80
    i_rhoA = PARAM_NAMES.index("rho_A")
    theta_start[i_rhoA] = 0.60   # mellom kj10 (0.39) og K&M (0.95)
    post_std[i_rhoA]    = 0.15   # bred proposal pga. ny identifikasjon
    ...
    seed        = 12
    print(f"  burnin={burnin_n}, max_recalib={max_recalib}, seed={seed}")
    i_sA = PARAM_NAMES.index("sigma_A")
    print(f"  sigma_A startpunkt: {theta_start[i_sA]:.4f} std={post_std[i_sA]:.4f}")
    print(f"  rho_A   startpunkt: {theta_start[i_rhoA]:.3f}")
...
out = {"summary": summary,
       "meta": {"n_samples": int(n_total),
                ...
                "begrunnelse": "PE-godkjent (2026-05-21): A6 sigma_A fri + A7 psi_W i mimicking rule"}}
...
print(f"\nNøkkelparametre (A6+A7-effekt):")
for nm in ["sigma_A", "rho_A", "rho_rp", "phi_I1", "sigma_rp", "psi_R", "psi_P1", "phi_u"]:
    sv = summary[nm]
