"""
[STAT] Fase 2 produksjon — phi_I1 fiksert til K&M=4.0 (PE-godkjent 2026-05-17).

19 frie parametre (phi_I1 fast = 4.0, sigma_A fast = 0.006).
5-blokks RWMH + logit-reparam for h_c og psi_R.
Start: Fase 2 5-blokks posterior (justert for 19 param).

Forventet effekt: sterkere pengepolitikktransmisjon (BNP-ratio mot NB: 0.26 → ~0.6).
"""
...
print(f"Fase 2 phi_I1-fix — {N_PARAMS} frie parametre")
print(f"phi_I1 = {PHI_I1_FIXED} (fast, K&M-verdi)")
print(f"Blokker: {{h_c,psi_R,psi_P1}} | AR | rho_Ys-klynge | sigma | {{phi_I2,phi_u}}")
...
# 5 blokker — phi_I1 er borte fra blk5
...
# Startpunkt: forrige kjøring (5-blokk, 19 av 20 param er gjenbrukbare)
...
chain, lp_vec, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.5, seed=11, verbose=True,
    save_prefix=str(ROT / "data" / "results" / "chain_fase2_phi1fix_prod"),
    use_reparam=True,
...
out_path = ROT / "data" / "results" / "chain_fase2_phi1fix_prod_posterior.json"
