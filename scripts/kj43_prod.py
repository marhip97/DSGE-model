"""
[STAT] kj43 — K&M 2019 fullkalibrering: psi_R=0.6663 + phi_I1=12.54

Funn fra kj41/kj42-analyse (2026-05-31):
  - psi_R treffer tak (0.95) for 3 kjøringer på rad — strukturelt identifikasjonsproblem
  - K&M 2019 (PDF kap. 18) har psi_R=0.6663, ikke 0.87 som tidligere antatt
  - phi_I1=12.5432 (K&M) vs vår kode 4.0→0.50 — stivere investeringer = saktere output
  - build_matrices_v3_forward har allerede E_t[π_{t+4}] og ψ_W=0.8705 aktive
  - Hypotese: psi_R≈0.95 i data fordi phi_I (for lavt) gir for rask investeringsrespons,
    som kompenseres med rentepersistens. K&M-kalibrering kan fikse dette.

Strategi kj43:
  - psi_R: N(0.6663, 0.001, [0.64, 0.69]) — fast kalibrert K&M 2019-verdi
  - phi_I1: N(12.5432, 0.001, [12.0, 13.0]) — fast kalibrert K&M 2019-verdi
  - phi_PQ=150 (beholder — ikke tilbake til 669 uten analyse)
  - rho_rp: N(0.17, 0.08, [0.05, 0.40]) — informativ fra kj41
  - psi_P1: N(0.2921, 0.05, [0.10, 0.60]) — mot K&M-verdi
  - gamma_p: N(0.75, 0.05, [0.55, 0.90]) — uendret
  - build_matrices_v3_forward + lambda_pi4=0.0
  - Warm start: kj41 posterior (klipps mot ny psi_R-prior)
"""
...
# ── Prior overrides ────────────────────────────────────────────────────────────
prior_overrides = {
    'psi_R':   ('normal', 0.6663, 0.001,  0.64,   0.69),    # K&M 2019 fast kalibrert
    'psi_P1':  ('normal', 0.2921, 0.05,   0.10,   0.60),    # mot K&M-verdi
    'phi_I1':  ('normal', 12.5432, 0.001, 12.0,   13.0),    # K&M 2019 fast kalibrert
    'gamma_p': ('normal', 0.75,   0.05,   0.55,   0.90),
    'rho_s':   ('normal', 0.03,   0.03,   0.00,   0.15),
    'rho_rp':  ('normal', 0.17,   0.08,   0.05,   0.40),    # informativ fra kj41
...
print(f"Warm start kj43 (psi_R={theta_start[idx_psiR]:.4f} K&M, phi_I1=12.54 K&M)")
print(f"  lp0={lp0:.2f}")
if irf0 is not None:
    r0 = nb_score(irf0)
...
logger.info(f"Warm start kj43 (psi_R={theta_start[idx_psiR]:.4f}, phi_I1=12.54)")
logger.info(f"lp0={lp0:.2f}")
logger.info("")
logger.info("=================================================================")
logger.info("  NEMO v3_forward — ADAPTIV MCMC kj43")
logger.info("  phi_PQ=150  lambda_pi4=0.0")
logger.info("  psi_R→N(0.6663,0.001,[0.64,0.69])  K&M 2019 FAST KALIBRERT")
logger.info("  phi_I1→N(12.5432,0.001,[12.0,13.0]) K&M 2019 FAST KALIBRERT")
logger.info("  psi_P1→N(0.2921,0.05,[0.10,0.60])  mot K&M-verdi")
logger.info("  rho_rp→N(0.17,0.08,[0.05,0.40])    informativ fra kj41")
logger.info("  Warm start: kj41  Prod=200,000  Burn-in=30,000  seed=43")
logger.info("=================================================================")
...
chain, lp_chain, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.70, seed=43, verbose=True,
    save_prefix=str(RESULTS / "chain_kj43_prod"),
    build_fn=_build_fn, prior_overrides=prior_overrides,
)

# ── Lagre ─────────────────────────────────────────────────────────────────────
np.save(RESULTS / "chain_kj43_prod.npy", chain)
np.save(RESULTS / "chain_kj43_prod_lp.npy", lp_chain)
...
json.dump(posterior, open(RESULTS / "chain_kj43_prod_posterior.json", "w"), indent=2)
json.dump(meta,      open(RESULTS / "chain_kj43_prod_meta.json",      "w"), indent=2)
...
if irf_post is not None:
    ...
    logger.info(f"\n=== kj43 ENDELIG EVALUERING (psi_R=0.6663 K&M, phi_I1=12.54 K&M) ===")
