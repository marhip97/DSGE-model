"""
[STAT] kj38 — Modell-konsistent fremoverskuende Taylor + phi_PQ=200

Strukturelle endringer fra kj35/kj36:
  - build_matrices_v3_forward: iterativ fixed-point løsning for E_t[π_{t+4}]
    Korreksjon av build_matrices_pi4chain (som kansellerte 97% av E_i-sjokket).
    R[I_R, E_i] = 0.97 (vs pi4chain: −0.03). NZ=49 uendret.
  - phi_PQ=200 (kappa_P=0.15): analytisk optimal fra punkt 4-sweep.
    PI.q4 → −0.152 (NB: −0.14) ✅ ved posterior mean.
  - lambda_pi4=0.0: ren fremoverskuende Taylor (E_t[π_{t+4}] only).

Analytiske funn (2026-05-30):
  - RMSE(korr, analytisk startpunkt): 0.2818 (forward+phi_PQ=200, psi_P1=0.29)
  - I_R undershoot uoppnåelig med v3-struktur — strukturell grense dokumentert.
  - B5: by4 ∈ [0.80, 1.50] ✅, bpi4 ≥ 0.35 ✅ ved startpunkt.

Strategi kj38:
  - phi_PQ=200 (fast kalibrering — ikke estimert)
  - lambda_pi4=0.0 (ren fremoverskuende Taylor)
  - psi_R: N(0.90, 0.015, [0.85, 0.97])
  - psi_P1: N(0.50, 0.20, [0.10, 2.00]) — utvidet, la data velge
  - rho_s: N(0.03, 0.03, [0.00, 0.15])
  - gamma_p: N(0.65, 0.05, [0.40, 0.85])
...
LOG     = RESULTS / "kj38_run.log"

# ── Strukturelle kalibreringer ────────────────────────────────────────────────
Parameters.phi_PQ = 200.0       # kappa_P = 6*5/200 = 0.150
LAMBDA_PI4 = 0.0                # ren fremoverskuende Taylor

def _build_fn(p, theta_H: float = 0.05):
    """Wrapper for build_matrices_v3_forward med lambda_pi4=0."""
...
theta_start[idx_rhos] = 0.03
post_std[idx_rhos]    = 0.03
theta_start[idx_psiR] = np.clip(theta_start[idx_psiR], 0.88, 0.94)
theta_start[idx_psiP1] = 0.50   # startverdi midt i utvidet prior
...
print(f"Warm start kj38 (phi_PQ=200, lambda_pi4=0, psi_P1={theta_start[idx_psiP1]:.2f})")
print(f"  lp0={lp0:.2f}")
if irf0 is not None:
    r0 = nb_score(irf0)
...
logger.info(f"Warm start kj38 (phi_PQ=200, lambda_pi4=0.0, psi_P1={theta_start[idx_psiP1]:.2f})")
logger.info(f"lp0={lp0:.2f}")
logger.info("")
logger.info("=================================================================")
logger.info("  NEMO v3_forward — ADAPTIV MCMC kj38")
logger.info("  phi_PQ=200 (kappa_P=0.15)  lambda_pi4=0.0 (ren fremoverskuende)")
logger.info("  psi_R→N(0.90,0.015)  psi_P1→N(0.50,0.20,[0.10,2.00])")
logger.info("  rho_s→N(0.03,0.03,[0,0.15])  Warm start: kj35")
logger.info("  Prod=200,000  Burn-in=30,000  seed=38")
logger.info("=================================================================")
...
chain, lp_chain, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.70, seed=38, verbose=True,
    save_prefix=str(RESULTS / "chain_kj38_prod"),
    build_fn=_build_fn, prior_overrides=prior_overrides,
)

# ── Lagre ─────────────────────────────────────────────────────────────────────
np.save(RESULTS / "chain_kj38_prod.npy", chain)
np.save(RESULTS / "chain_kj38_prod_lp.npy", lp_chain)

from nemo.estimation.mcmc import summarize_chain
posterior = summarize_chain(chain, lp_chain, PARAM_NAMES)
json.dump(posterior, open(RESULTS / "chain_kj38_prod_posterior.json", "w"), indent=2)
json.dump(meta,      open(RESULTS / "chain_kj38_prod_meta.json",      "w"), indent=2)
...
if irf_post is not None:
    ...
    logger.info(f"\n=== kj38 ENDELIG EVALUERING (phi_PQ=200, forward Taylor) ===")
    logger.info(f"RMSE(korr NB)={rmse_val:.4f}  B5: by4={by4:.3f} {'✅' if 0.80<=by4<=1.50 else '❌'}  bpi4={bpi4:.3f} {'✅' if bpi4>=0.35 else '❌'}")
    logger.info(f"  psi_R={s2['psi_R']['mean']:.4f}  psi_P1={s2['psi_P1']['mean']:.4f}  rho_s={s2['rho_s']['mean']:.4f}")
