"""
[STAT] Fase 2 phi_B-fix — gjeldselastisk UIP-premie aktivert (PE-godkjent 2026-05-19).

φ_B = 0.0016 (K&M Tabell 8) lagt til UIP-ligningen i equations.py.
Hypotese: σ_rp faller fra 0.017 mot K&M=0.006 når valutakursen har et
gjeldselastisk ankerfeste og ikke trenger å absorbere all uforklart dynamikk.

18 frie parametre (sigma_A, phi_I1, h_c alle faste).
Startpunkt: kjøring 3 posterior (siste gode kjøring med RER i observasjonssettet).
RER er tilbake i observasjonssettet (build_H standard).
...
print(f"Fase 2 phi_B-fix — {N_PARAMS} frie parametre")
print(f"phi_I1={PHI_I1_FIXED} (fast), h_c={H_C_FIXED} (fast, K&M)")
print(f"phi_B=0.0016 i UIP-ligning (K&M Tabell 8, PE-godkjent 2026-05-19)")
print(f"RER: tilbake i observasjonssettet")
...
# Startpunkt fra kjøring 3 (siste kjøring med RER og full posterior)
...
chain, lp_vec, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.5, seed=13, verbose=True,
    save_prefix=str(ROT / "data" / "results" / "chain_fase2_phib_prod"),
    use_reparam=True,
...
out = {"summary": summary,
       "meta": {**meta,
                ...
                "modellfix": ["A4a","A4c","CEE","A5","LTV-fortegn-E3E4",
                              "h_c-kalibrering","phi_B-UIP"],
                "begrunnelse": "PE-godkjent (2026-05-19): φ_B=0.0016 i UIP (K&M Tabell 8)",
                "psrf_max": float(np.nanmax(psrf)),
                "ess_min":  float(min(ess))}}

out_path = ROT / "data" / "results" / "chain_fase2_phib_prod_posterior.json"
...
print(f"\nsigma_rp posterior: {sigma_rp_mean:.4f}  (K&M: 0.006, kj3: 0.017)")
if sigma_rp_mean < 0.010:
    print("  → sigma_rp < 0.010 — φ_B-fix BEKREFTET ✓")
elif sigma_rp_mean < 0.014:
    print("  → sigma_rp redusert — delvis effekt")
else:
    print("  → sigma_rp fortsatt > 0.014 — ytterligere strukturell endring nødvendig")
