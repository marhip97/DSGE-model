"""
[STAT] Fase 2 C3-fix — sigma_rp fiksert til K&M=0.006 (PE-godkjent 2026-05-18).

18 frie parametre (sigma_A, phi_I1, sigma_rp alle faste).
psi_R prior utvidet: Beta(2,2) på (0.01, 0.92) (PE-godkjent 2026-05-18).

Forventet effekt: BNP-ratio mot NB Memo 3/2024 forbedres fra 6.3x til ~1.5-2x.
"""
...
from nemo.estimation.mcmc import (
    PARAM_NAMES, N_PARAMS, KM, PHI_I1_FIXED, SIGMA_RP_FIXED,
    build_H, build_Sv, log_posterior,
    adaptive_mcmc_with_monitoring, compute_psrf, compute_ess,
)

print(f"Fase 2 C3-fix — {N_PARAMS} frie parametre")
print(f"phi_I1={PHI_I1_FIXED} (fast), sigma_rp={SIGMA_RP_FIXED} (fast, K&M)")
...
blk4 = idx("sigma_C", "sigma_O", "sigma_i", "sigma_P", "sigma_H")
blk5 = idx("phi_I2", "phi_u")
blocks = [blk1, blk2, blk3, blk4, blk5]

# Startpunkt fra postfix-posterior, klamp psi_R og fjern sigma_rp
...
# psi_R: forrige posterior 0.842 er innenfor ny prior (0.01, 0.92) — beholdes
i_psi = PARAM_NAMES.index("psi_R")
print(f"psi_R startpunkt: {theta_start[i_psi]:.4f} (innenfor ny prior 0.92)")
...
chain, lp_vec, meta = adaptive_mcmc_with_monitoring(
    ...
    save_prefix=str(ROT / "data" / "results" / "chain_fase2_c3fix_prod"),
    use_reparam=True,
...
out = {"summary": summary,
       "meta": {**meta,
                ...
                "prior_endringer": [
                    ...
                    "sigma_rp: fjernet fra estimering, fast=0.006"
                ],
                "psrf_max": float(np.nanmax(psrf)),
                "ess_min":  float(min(ess))}}

out_path = ROT / "data" / "results" / "chain_fase2_c3fix_prod_posterior.json"
