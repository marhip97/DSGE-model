"""
[STAT] kj39A — Dogmatisk psi_R=0.88 + phi_PQ=200 (backward-looking Taylor)

FEVD-funn (kj38, 2026-05-30):
  - I_R-variasjon forklares 87.5% av konsumsjokk, bare 11.3% av pengepolitikk
  - psi_R er identifisert via Taylorregel-respons på konsumsjokk, ikke pengepol.
  - Likelihood stiger monotont mot unit root — ingen indre maksimum for psi_R

Strategi kj39A:
  - Dogmatisk prior psi_R ~ N(0.88, 0.005, [0.85, 0.91])
    Aksepterer ΔLL ≈ −437 likelihoodstraff for å matche NB-benchmark
  - phi_PQ=200 (kappa_P=0.15) beholdes — forbedrer PI.q4
  - build_matrices_v3 (backward-looking) — fremoverskuende Taylor hjalp ikke
  - Alle andre priors som kj38
  - Warm start: kj38 posterior (klipp psi_R→0.88)

Forventet: psi_R≈0.88 → halvtid≈5 kv → I_R.q12 nærmer seg 0 eller negativt
"""
...
# Klipp psi_R mot 0.88 (dogmatisk startpunkt)
theta_start[idx_psiR]  = 0.88
post_std[idx_psiR]     = 0.005   # tett — hjelper kjeden til å starte riktig

# ── Prior overrides ────────────────────────────────────────────────────────────
prior_overrides = {
    'psi_R':   ('normal', 0.88, 0.005, 0.85, 0.91),   # DOGMATISK — NB-konsistent
...
print(f"Warm start kj39A (phi_PQ=200, backward-looking, psi_R={theta_start[idx_psiR]:.3f})")
print(f"  lp0={lp0:.2f}")
if irf0 is not None:
    r0 = nb_score(irf0)
...
logger.info(f"Warm start kj39A (phi_PQ=200, backward-looking, psi_R={theta_start[idx_psiR]:.3f})")
logger.info(f"lp0={lp0:.2f}")
logger.info("")
logger.info("=================================================================")
logger.info("  NEMO v3 — ADAPTIV MCMC kj39A")
logger.info("  phi_PQ=200 (kappa_P=0.15)  backward-looking Taylor")
logger.info("  psi_R→N(0.88,0.005,[0.85,0.91]) DOGMATISK — NB-konsistent")
logger.info("  Warm start: kj38  Prod=200,000  Burn-in=30,000  seed=39")
logger.info("=================================================================")
...
chain, lp_chain, meta = adaptive_mcmc_with_monitoring(
    ...
    save_prefix=str(RESULTS / "chain_kj39A_prod"),
    build_fn=None, prior_overrides=prior_overrides,
)

# ── Lagre ─────────────────────────────────────────────────────────────────────
np.save(RESULTS / "chain_kj39A_prod.npy", chain)
np.save(RESULTS / "chain_kj39A_prod_lp.npy", lp_chain)
...
json.dump(posterior, open(RESULTS / "chain_kj39A_prod_posterior.json", "w"), indent=2)
json.dump(meta,      open(RESULTS / "chain_kj39A_prod_meta.json",      "w"), indent=2)
...
if irf_post is not None:
    ...
    logger.info(f"\n=== kj39A ENDELIG EVALUERING (dogmatisk psi_R=0.88) ===")
