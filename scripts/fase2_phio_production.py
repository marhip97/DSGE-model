"""
[STAT] Fase 2 phi_O-fix — direkte olje-valuta-kanal i UIP (PE-godkjent 2026-05-20).

phi_O=0.15 lagt til UIP: høy oljepris → NOK-appresiering (RER ned).
Kalibrert fra historisk NOK/olje-korrelasjon ~0.7:
  +10% oljepris → ~1.5% NOK-appresiering → phi_O ≈ 0.15

Hypotese: sigma_rp=0.017 skyldes at modellen absorberer olje-valuta-kanalen
i risikopremiesjokket. Med phi_O på plass bør sigma_rp falle mot K&M=0.006.

18 frie parametre (sigma_A, phi_I1, h_c faste). RER i observasjonssettet.
Startpunkt: kjøring 3 posterior.
...
print(f"Fase 2 phi_O-fix — {N_PARAMS} frie parametre")
print(f"phi_I1={PHI_I1_FIXED} (fast), h_c={H_C_FIXED} (fast, K&M)")
print(f"phi_O={phi_O} i UIP (olje-valuta-kanal, PE-godkjent 2026-05-20)")
...
chain, lp_vec, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.5, seed=55, verbose=True,
    save_prefix=str(ROT / "data" / "results" / "chain_fase2_phio_prod"),
    use_reparam=True,
...
out = {"summary": summary,
       "meta": {**meta,
                ...
                "modellfix": ["A4a","A4c","CEE","A5","LTV-fortegn-E3E4",
                              "h_c-kalibrering","phi_B-UIP","phi_O-UIP"],
                "begrunnelse": "PE-godkjent (2026-05-20): phi_O=0.15 olje-valuta-kanal i UIP",
                "psrf_max": float(np.nanmax(psrf)),
                "ess_min":  float(min(ess))}}

out_path = ROT / "data" / "results" / "chain_fase2_phio_prod_posterior.json"
...
if sigma_rp_mean < 0.010:
    print("  → sigma_rp < 0.010 — phi_O-kanal BEKREFTET ✓ — Alt. B løser problemet")
elif sigma_rp_mean < 0.014:
    print("  → sigma_rp redusert — delvis effekt, phi_O kalibrering kan justeres")
else:
    print("  → sigma_rp > 0.014 — phi_O-kanal ikke tilstrekkelig")
