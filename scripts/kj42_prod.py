"""
[STAT] kj42 — psi_R fast kalibrert til K&M 2019-verdi (0.87)

Funn fra kj41 (2026-05-31):
  - psi_R=0.9490 (treffer tak 0.95 for 3. kjøring på rad)
  - RMSE=0.2771 (identisk med kj40 — phi_PQ=125 ga ingen RMSE-gevinst)
  - I_R.q4=0.793 (NB: 0.55), I_R.q12=+0.447 (NB: -0.15) — for persistent
  - PI.q4=-0.272 (NB: -0.14) — umiddelbar PI-respons for stor
  - rho_rp=0.170 (falt fra kj40 0.329 — phi_PQ-endring endret kanal)

Diagnose: psi_R-identifikasjon er strukturell. Data vil ha psi_R→1 for å
matche post-COVID rentepersistens. Dogmatisk prior N(0.91,0.008) ikke nok.

Strategi kj42:
  - psi_R: N(0.87, 0.001, [0.85, 0.89]) — fast kalibrert mot K&M 2019-verdi
  - phi_PQ=150 (tilbake — phi_PQ=125 ga ikke bedre RMSE i kj41)
  - rho_rp: N(0.17, 0.08, [0.05, 0.40]) — informativ fra kj41-posterior
  - psi_P1: N(0.60, 0.15, [0.10, 2.00]) — uendret fra kj41
  - gamma_p: N(0.75, 0.05, [0.55, 0.90]) — uendret
  - build_matrices_v3_forward + lambda_pi4=0.0
  - Warm start: kj41 posterior, psi_R klippt til [0.86, 0.88]
"""
...
# Klipper psi_R mot ny prior-senter 0.87
theta_start[idx_psiR] = np.clip(theta_start[idx_psiR], 0.86, 0.88)
post_std[idx_psiR]    = 0.001

# ── Prior overrides ────────────────────────────────────────────────────────────
prior_overrides = {
    'psi_R':   ('normal', 0.87, 0.001, 0.85, 0.89),    # fast kalibrert — K&M 2019
    'psi_P1':  ('normal', 0.60, 0.15,  0.10, 2.00),
    'gamma_p': ('normal', 0.75, 0.05,  0.55, 0.90),
    'rho_s':   ('normal', 0.03, 0.03,  0.00, 0.15),
    'rho_rp':  ('normal', 0.17, 0.08,  0.05, 0.40),    # informativ fra kj41-posterior
...
print(f"Warm start kj42 (phi_PQ=150, psi_R={theta_start[idx_psiR]:.3f}, rho_rp={theta_start[idx_rhorp]:.3f})")
print(f"  lp0={lp0:.2f}")
if irf0 is not None:
    r0 = nb_score(irf0)
...
logger.info(f"Warm start kj42 (phi_PQ=150, psi_R={theta_start[idx_psiR]:.3f}, rho_rp={theta_start[idx_rhorp]:.3f})")
logger.info(f"lp0={lp0:.2f}")
logger.info("")
logger.info("=================================================================")
logger.info("  NEMO v3_forward — ADAPTIV MCMC kj42")
logger.info("  phi_PQ=150 (kappa_P=0.20)  lambda_pi4=0.0")
logger.info("  psi_R→N(0.87,0.001,[0.85,0.89])  FAST KALIBRERT K&M 2019")
logger.info("  rho_rp→N(0.17,0.08,[0.05,0.40])  informativ fra kj41")
logger.info("  psi_P1→N(0.60,0.15,[0.10,2.00])  uendret")
logger.info("  Warm start: kj41  Prod=200,000  Burn-in=30,000  seed=42")
logger.info("=================================================================")
...
chain, lp_chain, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.70, seed=42, verbose=True,
    save_prefix=str(RESULTS / "chain_kj42_prod"),
    build_fn=_build_fn, prior_overrides=prior_overrides,
)

# ── Lagre ─────────────────────────────────────────────────────────────────────
np.save(RESULTS / "chain_kj42_prod.npy", chain)
np.save(RESULTS / "chain_kj42_prod_lp.npy", lp_chain)
...
json.dump(posterior, open(RESULTS / "chain_kj42_prod_posterior.json", "w"), indent=2)
json.dump(meta,      open(RESULTS / "chain_kj42_prod_meta.json",      "w"), indent=2)
...
if irf_post is not None:
    ...
    logger.info(f"\n=== kj42 ENDELIG EVALUERING (phi_PQ=150, psi_R FAST 0.87) ===")
