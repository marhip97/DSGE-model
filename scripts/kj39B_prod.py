"""
[STAT] kj39B — phi_PQ=150 + gamma_p høyere prior + psi_R~N(0.90,0.01)

Funn fra kj39A (2026-05-30):
  - psi_R=0.902 (data aksepterer 0.90 med dogmatisk prior) — ikke ved tak
  - I_R.q4 forbedret (+0.650≈NB +0.55) ✅
  - Nytt problem: RER reverserer for raskt (q4: −0.641 vs NB −1.00)
  - RMSE=0.283 (beste hittil)

Analytiske funn (sweep 2026-05-30):
  - phi_PQ=150 (vs 200): marginalt bedre RER.q1, bedre PI.q4 balanse
  - gamma_p=0.85 (vs 0.62): bedre PI-persistens q8/q12, RMSE −0.004
  - Kombinasjon psi_R=0.92 + phi_PQ=150 + gamma_p=0.85: RMSE=0.2714 analytisk

Strategi kj39B:
  - phi_PQ=150 (kappa_P=6*5/150=0.20) — litt mer prisfriksjon enn kj38
  - psi_R: N(0.90, 0.010, [0.85, 0.95]) — litt bredere enn kj39A, rundt data-optimal
  - gamma_p: N(0.75, 0.05, [0.55, 0.90]) — høyere senter (vs kj39A N(0.65,0.05))
  - build_matrices_v3_forward + lambda_pi4=0.0 (som kj38)
...
# ── Prior overrides ────────────────────────────────────────────────────────────
prior_overrides = {
    'psi_R':   ('normal', 0.90, 0.010, 0.85, 0.95),   # litt bredere enn kj39A
    'psi_P1':  ('normal', 0.50, 0.20,  0.10, 2.00),
    'gamma_p': ('normal', 0.75, 0.05,  0.55, 0.90),   # høyere senter — bedre PI-persistens
...
print(f"Warm start kj39B (phi_PQ=150, forward λ=0, psi_R={theta_start[idx_psiR]:.3f}, gamma_p={theta_start[idx_gammap]:.3f})")
print(f"  lp0={lp0:.2f}")
if irf0 is not None:
    r0 = nb_score(irf0)
...
logger.info(f"Warm start kj39B (phi_PQ=150, forward λ=0, psi_R={theta_start[idx_psiR]:.3f}, gamma_p={theta_start[idx_gammap]:.3f})")
logger.info(f"lp0={lp0:.2f}")
logger.info("")
logger.info("=================================================================")
logger.info("  NEMO v3_forward — ADAPTIV MCMC kj39B")
logger.info("  phi_PQ=150 (kappa_P=0.20)  lambda_pi4=0.0")
logger.info("  psi_R→N(0.90,0.010,[0.85,0.95])")
logger.info("  gamma_p→N(0.75,0.05,[0.55,0.90])  høyere senter")
logger.info("  Warm start: kj39A  Prod=200,000  Burn-in=30,000  seed=39")
logger.info("=================================================================")
...
chain, lp_chain, meta = adaptive_mcmc_with_monitoring(
    ...
    scale_init=0.70, seed=39, verbose=True,
    save_prefix=str(RESULTS / "chain_kj39B_prod"),
    build_fn=_build_fn, prior_overrides=prior_overrides,
)

# ── Lagre ─────────────────────────────────────────────────────────────────────
np.save(RESULTS / "chain_kj39B_prod.npy", chain)
np.save(RESULTS / "chain_kj39B_prod_lp.npy", lp_chain)
...
json.dump(posterior, open(RESULTS / "chain_kj39B_prod_posterior.json", "w"), indent=2)
json.dump(meta,      open(RESULTS / "chain_kj39B_prod_meta.json",      "w"), indent=2)
...
if irf_post is not None:
    ...
    logger.info(f"\n=== kj39B ENDELIG EVALUERING (phi_PQ=150, gamma_p høyere) ===")
    logger.info(f"RMSE(korr NB)={rmse_val:.4f}  B5: by4={by4:.3f} {'✅' if 0.80<=by4<=1.50 else '❌'}  bpi4={bpi4:.3f} {'✅' if bpi4>=0.35 else '❌'}")
    logger.info(f"  psi_R={s2['psi_R']['mean']:.4f}  gamma_p={s2['gamma_p']['mean']:.4f}  psi_P1={s2['psi_P1']['mean']:.4f}")
